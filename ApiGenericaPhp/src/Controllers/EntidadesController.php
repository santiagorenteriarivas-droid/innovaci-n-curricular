...
class EntidadesController
{
    ...
    {
        try {
            // Soportar estructuras tanto planas como anidadas (ej. {"datos": {...}})
            $payload = isset($datos['datos']) && is_array($datos['datos']) ? $datos['datos'] : $datos;

            $resultado = $this->servicioCrud->verificarContrasenaAsync(
                $tabla,
                $esquema,
                $payload['campoUsuario'] ?? '',
                $payload['campoContrasena'] ?? '',
                $payload['valorUsuario'] ?? '',
                $payload['valorContrasena'] ?? ''
            );
