...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($accionPost === 'actualizar') {
        ...
        // Unimos las dos llaves para mandarlas al backend genérico
        $clavesUnidas = $clave . ',' . $claveSecundaria;
        $valoresUnidos = $valor . ',' . $valorSecundario;

        $resultado = $api->actualizar($tabla, $clavesUnidas, $valoresUnidos, $datos);
        $_SESSION['mensaje'] = $resultado['mensaje'];
        $_SESSION['tipo'] = $resultado['exito'] ? 'success' : 'danger';
    }
    ...
    if ($accionPost === 'eliminar') {
        ...
        $resultado = $api->eliminar($tabla, $clavesUnidas, $valoresUnidos);
        $_SESSION['mensaje'] = $resultado['mensaje'];
        $_SESSION['tipo'] = $resultado['exito'] ? 'success' : 'danger';
    }
...
foreach ($areasConocimiento as $a) { $mapaAreasConocimiento[$a['id'] ?? ''] = $a['area'] ?? 'Sin nombre'; }
?>
